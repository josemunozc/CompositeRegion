...
void Heat_Pipe<dim>::assemble_system_interval_temperature(const typename DoFHandler<dim>::active_cell_iterator &begin,
	...
	for (cell=begin; cell!=end; ++cell)
	{
		...
		double thermal_conductivity  = parameters.soil_thermal_conductivity;
		double thermal_heat_capacity = parameters.soil_specific_heat_capacity;
		double density               = parameters.soil_density;

		double cell_center = cell->center()[0];
		if (cell_center <= -1.* parameters.insulation_depth &&
				cell_center >= -1.*(parameters.insulation_depth + parameters.insulation_thickness))
		{
			thermal_conductivity  = parameters.insulation_thermal_conductivity;
			thermal_heat_capacity = parameters.insulation_specific_heat_capacity;
			density               = parameters.insulation_density;
		}
...
void Heat_Pipe<dim>::initial_condition_temperature()
{
	 ...
	 */
	std::cout << "Available initial condition entries: " << initial_condition.size()    << std::endl
			<< "Initial condition: \n\tDepth\tTemperature (C)\n";
	for (unsigned int i=0; i<initial_condition.size(); i++)
...
void Heat_Pipe<dim>::run()
{
			...
			time+=time_step, ++timestep_number)
	{
		...
		std::cout << "\tTa: " << std::setw(7) << std::setfill(' ')
		<< std::setprecision(2) << new_surface_temperature
		<< std::endl;
